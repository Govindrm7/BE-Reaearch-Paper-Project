...
imgsFolder = 'F:\Project1\images\New Dataset\CLAHE-RMSHE\Tumor';

listing = dir(fullfile(imgsFolder, '/*.jpg'));

for idx = 1:length(listing)
    disp(idx);
    s =  imread(fullfile(listing(idx).folder, listing(idx).name));
...
folder = strcat('F:\Project1\images\New Dataset\Detection\CLAHE-RMSHE\Tumor\',num2str(idx),'.jpg');
imwrite(tumorOutlineInserted, folder);
